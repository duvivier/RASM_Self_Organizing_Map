...
integer :: this_dy_cnt               ! For the arrays that hold the individual
                                     ! value for each day
integer, dimension(my_x,my_y) :: cnt ! How often each node occurs
...
double precision, dimension(my_len) :: som_node,input_day,bias,mse,mae

! Below are for the average of these variables for each node of the SOM
double precision, dimension(my_x,my_y) :: bias_arr,mae_arr,mse_arr,rmse_arr,corr_arr
double precision, dimension(my_x,my_y,1000) :: bias_ind_arr,mae_ind_arr,mse_ind_arr
!double precision, dimension(my_x,my_y,1000) :: rmse_ind_arr,corr_ind_arr

! Below are the SOM nodes in array format
double precision, dimension(my_len,my_x,my_y) :: som_node_array 
...
! Average mean absolute and squared error and bias, and root mean square error
   mae_sum=mae_sum/float(my_len)
   bias_sum=bias_sum/float(my_len)
   mse_sum=mse_sum/float(my_len)
   rmse=sqrt(mse_sum)
...
! Correlation calculation
   SSxx = sumx2 - ((sumx*sumx)/float(my_len))
   SSyy = sumy2 - ((sumy*sumy)/float(my_len))
   SSxy = sumxy - ((sumx*sumy)/float(my_len))
...
do j=1,my_y
   write(15,32) (rmse_arr(i,j),i=1,my_x)
   write(16,33) (bias_arr(i,j),i=1,my_x)
   write(17,32) (corr_arr(i,j),i=1,my_x)
   write(18,34) (mse_arr(i,j),i=1,my_x)
   write(19,32) (mae_arr(i,j),i=1,my_x)
enddo
